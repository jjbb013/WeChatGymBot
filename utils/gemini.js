// utils/gemini.js
const { parseFitnessLog } = require('./parser.js');

const API_URL = 'https://wfyelvimfelx.ap-southeast-1.clawcloudrun.com/v1/chat/completions';
const API_KEY = 'sk-willpan';

// 恢复了之前的系统提示词
const SYSTEM_PROMPT = `你是一个专业的健身教练AI助手。你的任务是分析用户的对话，并从中提取结构化的健身记录。
当用户描述一个训练动作时，你需要识别出以下关键信息：
- action (动作名称): 字符串
- reps (次数): 数字
- weight (负重): 数字, 如果未提及，默认为 0

你的回复必须遵循以下规则：
1.  如果用户的输入是有效的健身记录，你只能回复一个JSON对象，格式如下：
    {"type": "log", "data": {"action": "动作名称", "reps": 10, "weight": 50}}
2.  如果用户的输入是请求数据汇总（例如“总结一下我这周的训练”），你需要识别出时间范围（today, week, month, quarter），并回复一个JSON对象，格式如下：
    {"type": "summary", "data": {"period": "week"}}
3.  如果用户的输入既不是健身记录，也不是数据汇总请求（例如打招呼、问问题），你也要回复一个JSON对象，格式如下：
    {"type": "chat", "data": "你好！可以这样对我说话：\\n- “我刚刚练了20个深蹲，负重50公斤。”\\n- “我又做了10次，负重30kg。”\\n- “10次（我会自动识别上次的健身项目继续记录）。”\\n- 请汇总一下我今天/本周/本月的健身记录。"}
4.  不要在JSON之外添加任何额外的文字、解释或注释。`;

/**
 * 结合本地解析和Gemini API来解析用户输入
 * @param {string} userInput - 用户的输入文本
 * @param {object|null} lastLog - 上一次的健身记录
 * @returns {Promise<object>} - 返回解析后的JSON对象
 */
function getStructuredDataFromGemini(userInput, lastLog = null) {
  return new Promise((resolve, reject) => {
    // 预处理输入，移除末尾的句号和点，并去除首尾空格
    const text = userInput.trim().replace(/[.。]$/, '');

    // --- 混合策略 ---
    // 1. 首先尝试本地快速解析
    const summaryMatch = text.match(/^(今天|本周|本月|本季度)总结$/);
    if (summaryMatch) {
      const periodMap = { '今天': 'today', '本周': 'week', '本月': 'month', '本季度': 'quarter' };
      return resolve({ type: 'summary', data: { period: periodMap[summaryMatch[1]] } });
    }

    const localParsedData = parseFitnessLog(text, lastLog);
    if (localParsedData) {
      return resolve({ type: 'log', data: localParsedData });
    }

    // 2. 如果本地解析失败，则调用Gemini API
    let dynamicSystemPrompt = SYSTEM_PROMPT;
    if (lastLog) {
      dynamicSystemPrompt += `
      
      这是用户上一次的训练记录:
      - 动作: ${lastLog.action}
      - 重量: ${lastLog.weight}kg
      
      现在，请根据这个上下文处理用户的最新输入。规则如下：
      1. 如果用户只提供次数（例如“我又做了15个”或直接输入“15”），你应该使用上一次的动作和重量，只更新次数。
      2. 如果用户提供了新的重量和次数（例如“20公斤 12个”），你应该使用上一次的动作，但更新重量和次数。
      3. 如果用户提供了全新的完整记录（例如“弯举 10次 30kg”），则忽略上一次的记录，直接解析新记录。
      `;
    }

    wx.request({
      url: API_URL,
      method: 'POST',
      header: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${API_KEY}`
      },
      data: {
        model: "gemini-2.5-flash", // 恢复使用 gemini-2.5-flash 模型
        messages: [
          { role: "system", content: dynamicSystemPrompt },
          { role: "user", content: text } // 使用清洗后的文本
        ],
        temperature: 0.1,
        response_format: { type: "json_object" }
      },
      success: (res) => {
        if (res.statusCode === 200 && res.data.choices && res.data.choices.length > 0) {
          try {
            let content = res.data.choices[0].message.content;
            const jsonMatch = content.match(/```json\s*([\s\S]*?)\s*```/);
            if (jsonMatch && jsonMatch[1]) {
              content = jsonMatch[1];
            }
            const structuredData = JSON.parse(content);
            resolve(structuredData);
          } catch (e) {
            console.error("JSON parsing error:", e, "Raw content:", res.data.choices[0].message.content);
            reject(new Error('Failed to parse Gemini response JSON.'));
          }
        } else {
          reject(new Error(`Gemini API Error: ${res.statusCode} - ${JSON.stringify(res.data)}`));
        }
      },
      fail: (err) => {
        reject(new Error(`Network request failed: ${err.errMsg}`));
      }
    });
  });
}

module.exports = {
  getStructuredDataFromGemini
};
